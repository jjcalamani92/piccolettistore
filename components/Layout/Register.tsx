...
export const Register = () => {
	...
	const onResiterSubmit = async ({ username, email, password }: FormRegister) => {
		setShowError(false)
	...
	return (
		<form
			onSubmit={handleSubmit(onResiterSubmit)}
			className="h-screen flex bg-gray-100 from-green-400 to-orange-900 w-full py-16 px-4"
		>
			<div className="flex flex-col items-center justify-center w-full">
				<div className="bg-white shadow rounded lg:w-1/3  md:w-1/2 w-auto  p-10">
					...
					<div>
						...
						/>
						<div>
							{errors.username && <span className="text-sm text-orange-500">{errors.username.message}</span>}
						</div>
					</div>
					<div>
					...
					<div className="mt-8">
						<button
							...
							className="focus:ring-2 focus:ring-offset-2 focus:ring-orange-600 text-sm font-semibold leading-none text-white focus:outline-none bg-orange-600 border rounded hover:bg-orange-700 py-4 w-full"
						>
							Register
						</button>
					</div>
